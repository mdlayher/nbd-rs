...
impl<'a> Frame<'a> {
    ...
    /// the expected magic and I/O type values of that frame.
    pub(crate) fn check(
        src: &'a mut Cursor<&[u8]>,
    ...
    /// message type and I/O type valeus to assert the cursor is in the correct
    ...
    pub(crate) fn parse(
        ...
        match mtype {
            ...
            MagicType::SimpleReply => {
                ...
                // TODO(mdlayher): deal with trailing bytes for read replies.
