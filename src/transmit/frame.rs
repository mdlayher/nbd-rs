...
impl<'a> Frame<'a> {
    ...
    /// the length of that
    pub fn check(src: &Cursor<&[u8]>) -> Result<()> {
    ...
    pub fn parse(src: &'a mut Cursor<&[u8]>) -> Result<Frame<'a>> {
    ...
    pub async fn write<S: AsyncWrite + Unpin>(self, dst: &mut S, buf: &[u8]) -> Result<Option<()>> {
        match self {
